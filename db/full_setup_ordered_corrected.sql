...
-- BLOC START
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'mamas' AND column_name = 'actif') THEN
    ALTER TABLE mamas ADD COLUMN actif boolean DEFAULT true;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'familles' AND column_name = 'actif') THEN
    ALTER TABLE familles ADD COLUMN actif boolean DEFAULT true;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'menu_fiches' AND column_name = 'actif') THEN
    ALTER TABLE menu_fiches ADD COLUMN actif boolean DEFAULT true;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'taches' AND column_name = 'actif') THEN
    ALTER TABLE taches ADD COLUMN actif boolean DEFAULT true;
  END IF;
