...
export default function FactureForm({
  ...
  const saving = saveFacture.isPending;
  ...
  const handleSubmit = async (e) => {
    ...
    const lignesPayload = lignes.map((l) => ({
      id: l.id,
      produit_id: l.produit_id ?? l.produit?.id ?? null,
    ...
    try {
      const res = await saveFacture.mutateAsync(payload);
      const newId = res?.id || res?.facture?.id || factureId;
      onSaved?.(newId);
