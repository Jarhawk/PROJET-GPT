...
export default function FicheForm({ initialData = null, onSave, onCancel }) {
  ...
  const [produits, setProduits] = useState([]); // {id, nom, type: "produit"|"sousrecette", unite, pmp, ficheDetail}
  const [searchValue, setSearchValue] = useState("");
  const [familles, setFamilles] = useState([]);
  const [unites, setUnites] = useState([]);
  const [loading, setLoading] = useState(false);

  // Modale imbriquée pour sous-recette
  ...
  useEffect(() => {
    ...
    fetchAll();
  }, [claims?.mama_id, openSousRecette]); // refresh liste après ajout sous-recette
  ...
  const handleSousRecetteCreated = (fiche) => {
    ...
    // Ajout direct à la liste des produits dispo
