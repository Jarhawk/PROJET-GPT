...
-- Summary: 107 policies, 35 functions OR REPLACE, 0 _v2 functions,
--          6 triggers, 89 constraints, 113 indexes verified.
...
do $$ begin
  if exists (
    ...
    drop policy requisitions_insert on public.requisitions;
  end if;
end $$;
do $$ begin
  if not exists (
...
do $$ begin
  if exists (
    select 1 from pg_policies
    where schemaname='public' and tablename='zones_stock' and policyname='zones_stock_all'
  ) then
    drop policy zones_stock_all on public.zones_stock;
  end if;
end $$;
do $$ begin
  if not exists (
...
grant select, insert, update, delete on public.zones_droits to authenticated;
...
CREATE OR REPLACE FUNCTION public.sync_pivot_from_produits()
RETURNS trigger
LANGUAGE plpgsql
AS $$
begin
...
CREATE OR REPLACE FUNCTION public.sync_produits_from_pivot()
RETURNS trigger
LANGUAGE plpgsql
AS $$
begin
...
-- -- CLEANUP (revue manuelle avant exécution) :
-- do $cleanup$
-- begin
--   -- Exemples connus (héritage/legacy) :
--   -- if exists (select 1 from information_schema.tables where table_schema='public' and table_name='mouvements') then
--   --   execute 'drop table if exists public.mouvements cascade';
--   -- end if;
--   -- if exists (select 1 from information_schema.tables where table_schema='public' and table_name='mouvements_centres_cout') then
--   --   execute 'drop table if exists public.mouvements_centres_cout cascade';
--   -- end if;
--   -- if exists (select 1 from pg_views where schemaname='public' and viewname='v_menu_engineering') then
--   --   execute 'drop view if exists public.v_menu_engineering cascade';
--   -- end if;
-- end
-- $cleanup$;
