...
begin
  if not exists (
    ...
    where tgname = 'set_ts_templates_cmd' and tgrelid = 'public.templates_commandes'::regclass
  ) then
    create trigger set_ts_templates_cmd before update on public.templates_commandes
...
begin
  if not exists (
    select 1 from pg_trigger
    where tgname = 'set_ts_zones' and tgrelid = 'public.zones_stock'::regclass
  ) then
...
begin
  if not exists (
    select 1 from pg_trigger
    where tgname = 'trg_pz_ts' and tgrelid = 'public.produits_zones'::regclass
  ) then
...
begin
  if not exists (
    select 1 from pg_trigger
    where tgname = 'trg_prod_sync_zone' and tgrelid = 'public.produits'::regclass
  ) then
    create trigger trg_prod_sync_zone after insert or update of zone_id on public.produits
...
begin
  if not exists (
    select 1 from pg_trigger
    where tgname = 'trg_pz_sync_prod' and tgrelid = 'public.produits_zones'::regclass
  ) then
    create trigger trg_pz_sync_prod after insert or update of actif on public.produits_zones
...
begin
  if not exists (
    ...
    create policy pz_select on public.produits_zones for select using (mama_id = current_user_mama_id());
  end if;
end $$;
...
begin
  if not exists (
    ...
    create policy pz_iud on public.produits_zones for all using (mama_id = current_user_mama_id())
      with check (mama_id = current_user_mama_id());
  end if;
end $$;
