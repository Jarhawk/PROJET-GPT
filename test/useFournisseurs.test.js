...
const insertSupplier = vi.fn(() => ({ select: selectAfterInsert }));
const insertContact = vi.fn(() => Promise.resolve({ data: {}, error: null }));
const upsertContact = vi.fn(() => Promise.resolve({ data: {}, error: null }));
const updateSupplier = vi.fn(() => ({ eq: vi.fn(() => ({ eq: vi.fn(() => Promise.resolve({ error: null })) })) }));
...
const fromMock = vi.fn((table) => {
  if (table === 'fournisseurs') {
    return { insert: insertSupplier, update: updateSupplier, select: fetchQuery.select, eq: fetchQuery.eq, order: fetchQuery.order, range: fetchQuery.range, ilike: fetchQuery.ilike };
  }
...
test('createFournisseur insère un contact lié', async () => {
  ...
  expect(insertSupplier).toHaveBeenCalledWith([{ nom: 'Test', actif: true, mama_id: 'm1' }]);
...
test('updateFournisseur met à jour le contact', async () => {
  ...
  expect(updateSupplier).toHaveBeenCalledWith({ actif: false });
  expect(fromMock).toHaveBeenCalledWith('fournisseur_contacts');
